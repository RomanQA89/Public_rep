...
def _make_request(token, method_name, method='get', params=None, files=None):
    ...
    """
    if not token:
        raise Exception('CryptoBot token is not defined')
    if API_URL:
        # noinspection PyUnresolvedReferences
...
def set_game_score(token, user_id, score, force=None, disable_edit_message=None, chat_id=None, message_id=None,
    ...
    :param token: CryptoBot's token (you don't need to fill this)
...
def send_invoice(
    ...
    :param invoice_payload: CryptoBot-defined invoice payload, 1-128 bytes. This will not be displayed to the user, use for your internal processes.
...
def answer_shipping_query(token, shipping_query_id, ok, shipping_options=None, error_message=None):
    """
    If you sent an invoice requesting a shipping address and the parameter is_flexible was specified, the CryptoBot API will send an Update with a shipping_query field to the bot. Use this method to reply to shipping queries. On success, True is returned.
    :param token: CryptoBot's token (you don't need to fill this)
...
def answer_pre_checkout_query(token, pre_checkout_query_id, ok, error_message=None):
    """
    Once the user has confirmed their payment and shipping details, the CryptoBot API sends the final confirmation in the form of an Update with the field pre_checkout_query. Use this method to respond to such pre-checkout queries. On success, True is returned. Note: The CryptoBot API must receive an answer within 10 seconds after the pre-checkout query was sent.
    :param token: CryptoBot's token (you don't need to fill this)
